...
# Obtener el token defrom typing import Final
from telegram import Update
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes, CallbackContext
import json
import os
...
if __name__ == '__main__':
    ...
    app.run_polling(poll_interval=3)
            ...
            guardar_datos_usuario(user_id, datos[str(user_id)])
            await update.message.reply_text(f'Has reservado {cantidad}€ como ahorro. Total ahorrado: {datos[str(user_id)]["estalvi"]}€. Saldo actual: {datos[str(user_id)]["salary"]}€.')
        except ValueError:
            await update.message.reply_text('Por favor, introduce una cantidad válida.')
...
# Comando /quotesMensuales
async def quotes_mensuales_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    datos = obtener_datos_usuario(user_id)
    quotes = datos[str(user_id)]['quotes']
    await update.message.reply_text(f'Los gastos mensuales fijos son de {quotes}€.')

# Comando /introducirQuotesMensuales
async def introducir_quotes_mensuales_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    esperando_quotes.add(user_id)
    await update.message.reply_text('Introduce las nuevas cuotas mensuales que quieres agregar.')

async def manejar_quotes_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id in esperando_quotes:
        datos = obtener_datos_usuario(user_id)
        try:
            cantidad = float(update.message.text.replace('€', '').strip())
            datos[str(user_id)]['quotes'] = cantidad  # Actualizar las cuotas mensuales
            esperando_quotes.remove(user_id)
...
# Comando /gastosDiarios
async def gastos_diarios_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    datos = obtener_datos_usuario(user_id)
    gastos_diarios = datos[str(user_id)]['gastos_diaris']
    await update.message.reply_text(f'Los gastos diarios hasta ahora son de {gastos_diarios}€.')

# Comando /introducirGastosDiarios
async def introducir_gastos_diarios_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    esperando_gastos_diaris.add(user_id)
    await update.message.reply_text('Introduce la cantidad de gastos diarios que quieres agregar.')

async def manejar_gastos_diarios_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id in esperando_gastos_diaris:
        datos = obtener_datos_usuario(user_id)
        try:
            cantidad = float(update.message.text.replace('€', '').strip())
            datos[str(user_id)]['gastos_diaris'] += cantidad  # Sumar a gastos diarios
            esperando_gastos_diaris.remove(user_id)
            ...
            guardar_datos_usuario(user_id, datos[str(user_id)])
            await update.message.reply_text(f'Has añadido {cantidad}€ a tus gastos diarios. Total de gastos diarios: {datos[str(user_id)]["gastos_diaris"]}€. Saldo actual: {datos[str(user_id)]["salary"]}€.')
        except ValueError:
            await update.message.reply_text('Por favor, introduce una cantidad válida.')

# Comando /ganancias
async def ganancias_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    datos = obtener_datos_usuario(user_id)
    ganancies = datos[str(user_id)]['ganancies_esporadiques'] + datos[str(user_id)]['ganancies_mensuals']
    await update.message.reply_text(f'Tus ingresos totales son de {ganancies}€.')

# Comando /introducirGananciasEsporadicas
async def introducir_ganancias_esporadicas_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    esperando_ganancies_esporadiques.add(user_id)
    await update.message.reply_text('Introduce la cantidad de ingresos esporádicos que quieres agregar.')

async def manejar_ganancias_esporadicas_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id in esperando_ganancies_esporadiques:
        datos = obtener_datos_usuario(user_id)
        try:
            cantidad = float(update.message.text.replace('€', '').strip())
            datos[str(user_id)]['ganancies_esporadiques'] += cantidad  # Sumar a ingresos esporádicos
            esperando_ganancies_esporadiques.remove(user_id)
            ...
            guardar_datos_usuario(user_id, datos[str(user_id)])
            await update.message.reply_text(f'Has añadido {cantidad}€ a tus ingresos esporádicos. Total de ingresos: {datos[str(user_id)]["ganancies_esporadiques"]}€. Saldo actual: {datos[str(user_id)]["salary"]}€.')
        except ValueError:
            await update.message.reply_text('Por favor, introduce una cantidad válida.')

# Comando /introducirGananciasMensuales
async def introducir_ganancias_mensuales_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    esperando_ganancies_mensuals.add(user_id)
    await update.message.reply_text('Introduce la cantidad de ingresos mensuales que quieres agregar.')

async def manejar_ganancias_mensuales_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id in esperando_ganancies_mensuals:
        datos = obtener_datos_usuario(user_id)
        try:
            cantidad = float(update.message.text.replace('€', '').strip())
            datos[str(user_id)]['ganancies_mensuals'] += cantidad  # Sumar a ingresos mensuales
            esperando_ganancies_mensuals.remove(user_id)
            ...
            guardar_datos_usuario(user_id, datos[str(user_id)])
            await update.message.reply_text(f'Has añadido {cantidad}€ a tus ingresos mensuales. Total de ingresos: {datos[str(user_id)]["ganancies_mensuals"]}€. Saldo actual: {datos[str(user_id)]["salary"]}€.')
        except ValueError:
            await update.message.reply_text('Por favor, introduce una cantidad válida.')
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler('saldo', saldo_))
    app.add_handler(CommandHandler('reserva', reserva_))
    app.add_handler(CommandHandler('introducir_reserva', introducir_reserva_))
    app.add_handler(CommandHandler('despeses', despeses_))
    app.add_handler(CommandHandler('quotesMensuales', quotes_mensuales_))
    app.add_handler(CommandHandler('introducirQuotesMensuales', introducir_quotes_mensuales_))
    app.add_handler(CommandHandler('gastosDiarios', gastos_diarios_))
    app.add_handler(CommandHandler('introducirGastosDiarios', introducir_gastos_diarios_))
    app.add_handler(CommandHandler('ganancias', ganancias_))
    app.add_handler(CommandHandler('introducirGananciasEsporadicas', introducir_ganancias_esporadicas_))
    app.add_handler(CommandHandler('introducirGananciasMensuales', introducir_ganancias_mensuales_))

    # Respuestas
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_reserva_))
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_quotes_))
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_gastos_diarios_))
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_ganancias_esporadicas_))
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_ganancias_mensuales_))
